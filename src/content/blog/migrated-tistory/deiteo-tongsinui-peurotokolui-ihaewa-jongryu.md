...
| **반이중 통신만 지원- 단방향- 반이중(전송해야만 전달 가능)- 전이중** | 포인트 투, 포인트, 멀티-포인트만 지원 | Stop-and-Wait **ARQ(오류제어)** |
...
| **반이중과 전이중 통신만 지원- 전이중: 양방향이 동시에 전송이 가능한** | 포인트 투 포인트, 멀티-포인트만 지원 |
...
| --- |
| 전송방식 | 데이터 링크 형식 | 오류 제어 방식 |
| **단방향, 반이중,전이중 통신을 모두 지원** | 포인트 투 포인트,**멀티-포인트,****루프 방식 모두 지원** | **Go-back-N ARQ**데이터를 다량으로 보내서안된 부분만 재요청 |
